...
const OTHER_INFO = ["Wheelchair-accessible entrance", "Restroom", "Free Wi-Fi", "Family Friendly", "Air Conditioning"];
const PARKING = ["Valet Parking", "Paid Parking", "Private Parking Available", "Street Parking"];
const PAYMENTS = ["Cash", "Credit Card", "Debit Card", "Digital Wallet", "QR Scan", "Bank Transfer"];
...
export default function RegisterBusinessScreen({ navigation }: { navigation: any }) {
  ...
  const ensureMediaLibraryPermission = async () => {
    ...
    const granted = await PermissionsAndroid.request(permission, {
      ...
      message: "FoodHub needs your permission to pick images from your gallery.",
  ...
  const requestLocationPermission = async () => {
    ...
    const granted = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
      {
        ...
        message: "FoodHub uses your location to show your restaurant position.",
        buttonPositive: "Allow",
        buttonNegative: "Deny",
      }
    );
  ...
  const handleSubmit = async () => {
    try{
      const { username, email, password, confirmPassword, restaurantName, phone, whatsapp } = formState;
      
      //Mandatory fields checks
      if (!username || !email || !password || !confirmPassword || !restaurantName) {
      ...
      setTimeout(() => {
        ...
        const successMsg = "Restaurant registered successfully!";
        const uploadSummary = [
          `✓ Restaurant: ${restaurantName}`,
          `✓ Username: ${username}`,
          `✓ Email: ${email}`,
          mainPhoto ? `✓ Main photo uploaded` : `⚠ No main photo`,
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <FormSection title="Restaurant Information">
        <TextInput
          ...
          onChangeText={(t) => handleChange("restaurantName", t)}
        />
        <TextInput
        ...
        <Text style={styles.label}>Restaurant Photo {mainPhoto ? "✅" : ""}</Text>
         <TouchableOpacity style={styles.uploadBox} onPress={pickMainPhoto}>
          {mainPhoto?.uri ? (
            ...
            <Text style={{ color: "#666" }}>📸 Upload Restaurant Photo (PNG/JPG)</Text>
          )}
        </TouchableOpacity>
      ...
      {/*Parking Facilities */}
      <FormSection title="Parking Options">
        {PARKING.map((p) => (
      ...
      <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit} disabled={uploading}>
        {uploading ? (
          ...
          <Text style={styles.submitText}>Submit Restaurant Registration</Text>
        )}
      </TouchableOpacity>
      ...
      {uploading && (
        <View style={{ padding: 10, backgroundColor: "#f0f7ff", borderRadius: 8, marginTop: 10 }}>
          <Text style={{ color: "#0066cc", fontSize: 12, textAlign: "center" }}>
            📡 Processing restaurant registration... Please wait
          </Text>
        </View>
      )}
    </ScrollView>
